...
class TabuSearch:
    def __init__(self, solution, fitness_function, bap, move_tabu=30, swap_tabu=50, iters=100):
        self.tabu_move = []
        self.tabu_swap = []
        ...
        self.best_fitness = self.fitness_function(self.best_solution, self.bap)

    def get_best_fitness(self):
        return self.fitness_function(self.best_solution, self.bap)
    ...
    def get_neighbour(self):
        ...
        for v_id, vessels_to_move in enumerate(vessel_allowed_berths):
            for candidate_berth in vessels_to_move:
                ...
                if new_soln != False:
                    new_sol_fitness = self.fitness_function(new_soln, self.bap)
        ...
        sorted_neighbours = sorted(neighbourhood, key = lambda x: x.solution_fitness)
        ...
        while not placement_done:
            ...
            if isinstance(cand_move, MoveNeighbour):
                ...
                if v_id not in self.tabu_move:
                    self.tabu_move.append(v_id)
                    self.current_solution = eval(repr(cand_move.solution))
                    
                    if len(self.tabu_move) > self.move_tabu:
                        self.tabu_move = self.tabu_move[1:]
                    
                    if cand_move.solution_fitness <= self.best_fitness:
                        self.best_fitness = cand_move.solution_fitness
